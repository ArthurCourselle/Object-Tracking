...
from appearance_aware_tracker import AppearanceAwareTracker
...
def main():
    ...
    reid_model_path = os.path.join(script_dir, "../reid_osnet_x025_market1501.onnx")

    output_video_path = os.path.join(script_dir, "tracking_output_tp4.mp4")
    output_txt_path = os.path.join(script_dir, "ADL-Rundle-6_tracking_tp4.txt")
    ...
    tracker = AppearanceAwareTracker(
    ...
    print(f"Processing {len(img_files)} frames with Appearance-Aware IoU-Kalman Tracker.")
    ...
    for i, img_file in enumerate(img_files):
        ...
        if len(dets) > 0:
             tracked_objects = tracker.update(frame, dets)
        else:
             tracked_objects = tracker.update(frame, np.empty((0, 5)))
